...
export const getAllApplications = async (userId, statuses = []) => {
    // Build query string dynamically based on userId and statuses
    const statusQuery = statuses.length ? `status=${statuses.join(',')}` : '';
    const userQuery = userId ? `userId=${userId}` : '';  // Make sure to pass userId here
    const query = [userQuery, statusQuery].filter(Boolean).join('&'); // Combine queries with '&'
    const url = `${API}/applications${query ? `?${query}` : ''}`; // Construct the API endpoint
